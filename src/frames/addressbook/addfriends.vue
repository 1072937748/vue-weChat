<template>
  <section class="child_page">
    <head-top crossover="好友资料"></head-top>
    <div class="details">
      <div class="details_li">
        ...
        <div class="details_right">
          <div class="details_right_top">
            <span>{{searchResult.remarks ? searchResult.remarks : searchResult.name}}</span>
            <div class="sexsvg">
              <svg>
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href=" searchResult.sex == 0 ? '#boy' : '#girl' "></use>
              </svg>
            </div>
          </div>
          <div class="details_right_obt">
            账号：{{searchResult.phone}}
          </div>
        </div>
      </div>
      <!-- <div class="details_li">
				<router-link to='/addressbook/setInfo' class="setnote">
					设置备注
				</router-link>
			</div> -->
        ...
        <div class="person_photo">
          <router-link to='' class="details_person_a">
            ...
            <div class="person_photo_right clear">
              <img v-if="!gallery||gallery.length==0" src="default.jpg" alt="">
              <img :src="value" alt="" v-for="(value, i) in gallery" :key="i">
            </div>
          </router-link>
        </div>
      ...
      <div class="sendmessage">
        <!--  -->
        <router-link :to='routerLink' class="send" @click.native="enterdDialogue">
          添加为好友
        </router-link>
      </div>
    </div>
...
import { addfriend } from "src/service/getData.js";
...
export default {
  ...
  created() {},
  mounted() {
    this.gallery = this.searchResult.gallery;
  },
  components: {
    headTop
  },
  computed: {
    ...mapState(["userInfo", "searchResult"])
  },
  methods: {
    ...mapMutations(["SAVE_DIALOGUE", "SAVE_MESSAGE"]),
    enterdDialogue() {
      this.SAVE_MESSAGE(this.searchResult);
      addfriend({
        id: this.userInfo.id,
        friend: this.searchResult.id
      })
        .then(res => {
          if (res.status == 200) {
            this.$router.push({
