  ...
  <section>
    ...
    <section class="coversation" ref="groupHeight" @touchmove='loadMore'>
      <section class="coversationlist">
        <ul>
          <!--  群聊-->
          <li v-for="(item, i) in groupconversine" :key="i">
            <div class="other" :class="{mysay : item.user_id == userInfo.id }">
              ...
              <div class="whatsay">
                ...
                <div class="whatsay_text" v-if="item.content.includes(';base64,')">
                  <img :src="item.content" alt="链接失效">
                </div>
                <div class="whatsay_text" v-else-if="item.content.includes('emoji')">
                  <img :src="item.content.replace('emoji', '')" alt="链接失效">
                </div>
                <div class="whatsay_text" v-else v-html="item.content"></div>
              </div>
            </div>
          </li>
        </ul>
    ...
    <footer :class=" {footshow : clickmore}">
      <section class="foot_top">
        ...
        <div>
          ...
          <svg v-else @click="bottomShow('add')">
        ...
        <div class="swiper-container">
          <div class="swiper-wrapper">
            <div class="swiper-slide" v-for="(value,item) in chatData" :key="item">
              <ul class="clear" v-if="!isFace">
                <li v-for="(value, index) in value" :key="index">
                  ...
                  <input v-if="value.chatSvgname == '相册'" type="file" @change="chatSvgname" class="album">
                </li>
              </ul>
              <ul class="clear" v-else>
                <li v-for="(value, index) in emoji" :key="index" @click="sendEmoji(value.src)">
                  <div class="swiper_svg">
...
import emoji from "src/service/data/emoji";
...
export default {
  ...
  mounted() {
    ...
    socket.emit("enterGroup", {
      name: this.userInfo.remarkes ? this.userInfo.remarkes : this.userInfo.name
    });
    this.getUserInfo();
  ...
  computed: {
    ...mapState(["infor", "userInfo", "allgroup", "groupInfo", "unreadG"])
  },
  watch: {
    unreadG: {
      immediate: true,
      handler: function(val) {
        val.forEach((data, index) => {
          if (data.room_id == this.groupInfo.id) {
  ...
  methods: {
    ...
    ...mapMutations(["GET_ALLGROUP", "DIALOG_COVER", "CHANGE_UNREADG"]),

    async groupList(offset) {
      const groupData = await getHistory({
        offset: this.offset,
        limit: 20,
      ...
      if (groupData.status == 200) {
        ...
        for (let i = 0; i < groupData.history.length; i++) {
          if (!groupData.history[i].content) {
            //清空 空 数据
    ...
    chatSvgname(event) {
      ...
      if (!reader) {
        that.DIALOG_COVER("!your browser doesn't support fileReader");
        that.value = "";
        return;
      }
      reader.onload = e => {
