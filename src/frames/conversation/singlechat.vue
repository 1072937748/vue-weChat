  ...
  <section>
    <head-top :crossover="chatname">
      <section class="coversPart" slot="person">
        <router-link to='/addressbook/details' class="person_link">
    ...
    <section class="coversation" ref="singleHeight" @touchmove='loadMore'>
      <section class="coversationlist" @click="bottomHide">
        <ul>
          <li v-for="(item, i) in conversine" :key="i">
            <div class="other" :class="{mysay : item.sendobject == 0 }">
              ...
              <div class="whatsay">
                ...
                <div class="whatsay_text" v-html="item.Messageblob"></div>
              </div>
            </div>
          </li>
        </ul>
        ...
        <div class="swiper-container">
          <div class="swiper-wrapper">
            <div class="swiper-slide" v-for="(value,item) in chatData" :key="item">
              <ul class="clear">
                <li v-for="(value, i) in value" :key="i">
...
import { userWord, chatData, addDialog } from "src/service/getData";
...
export default {
  ...
  mounted() {
    ...
    this.from_id = this.userInfoData.id;
    this.to_id = this.infor.id;
    this.CHANGE_UNREAD(
      this.unread.filter(value => {
        return value.from_id == this.to_id;
      })
    );
    // 底部功能
    chatData()
    ...
    this.chatname = this.infor.remarks
      ? this.infor.remarks
      : this.infor.petname;
    // this.getUserInfo();
    this.getHistory();
  ...
  computed: {
    ...mapState(["infor", "unread", "userInfo", "contactList"])
  },
  watch: {
    //提取消息，并更改状态
    unread(val) {
      val.forEach((data, index) => {
        if (data.from_id == this.to_id) {
          try {
            const res = data;
            this.light = false;
            ...
            this.$nextTick(() => {
              window.scrollTo(
                0,
                this.$refs.singleHeight.offsetHeight - window.innerHeight
              );
            });
            socket.emit("updatechat", res);
            val.splice(index, 1);
            this.CHANGE_UNREAD([...val]);
          } catch (err) {
            this.DIALOG_COVER("获取聊天信息失败" + err);
          }
        }
      });
    }
  ...
  methods: {
    ...
    ...mapMutations(["DIALOG_COVER", "SAVE_DIALOGUE", "CHANGE_UNREAD"]),
    getHistory() {
      userWord({
        offset: this.offset,
        ...
        if (res.status == 200) {
          ...
          if (res.history.length) {
            ...
            res.history.forEach(i => {
              arr.push({
                avatar: i.avatar,
                sendobject: this.userInfo.id == i.from_id ? 0 : 1,
                Messageblob: i.content
              });
            });
            this.conversine = [...arr, ...this.conversine];
            this.$nextTick(() => {
              if (this.offset == 20) {
                window.scrollTo(0, this.$refs.singleHeight.offsetHeight);
              } else {
                window.scrollTo(
                  0,
                  this.$refs.singleHeight.offsetHeight - this.lastPageHeight
                );
              }
          ...
          this.lastPageHeight = this.$refs.singleHeight.offsetHeight;
        } else {
    ...
    loadMore() {
      ...
      if (this.scroll == 0 && !this.loadStatus) {
        this.loadStatus = true;
        this.getHistory();
      } else {
        this.loadStatus = false;
    ...
    async clickSend() {
      ...
      const message = {
        ...
        remarks: this.userInfoData.remarks,
        avatar: this.userInfoData.avatar,
        sendobject: 0,
        Messageblob: this.inputmessage
      };
      this.conversine.push(message);
      // 提交保存信息
      await socket.emit("singlechat", {
        from_id: this.from_id,
        to_id: this.to_id,
        name: this.userInfoData.name,
        remarks: this.userInfoData.remarks,
